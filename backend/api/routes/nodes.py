...
from models.database import Node, Session, Topic, User, NodeType, get_db
...
async def get_node(
    ...
    """Get a single node."""
    return await verify_node_ownership(node_id, current_user, db)
...
async def get_node_children(
    ...
    await verify_node_ownership(node_id, current_user, db)
    result = await db.execute(
        select(Node)
...
async def select_branch(
    ...
    """Select this node as the active branch."""
    node = await verify_node_ownership(node_id, current_user, db)
...
async def add_note(
    ...
    parent_node = await verify_node_ownership(node_id, current_user, db)
...
async def get_side_chat_threads(
    ...
    result = await db.execute(
        ...
        .order_by(func.min(Node.created_at))
    )
    rows = result.all()
    return [
        {
...
async def get_side_chats(
    ...
    """Get side chat messages for a node."""
    await verify_node_ownership(node_id, current_user, db)
...
async def get_session_nodes(
    ...
    """Get all root-level nodes for a session."""
